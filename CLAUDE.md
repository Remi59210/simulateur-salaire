...
| Indicateur | Valeur |
|---|---|
| Pages HTML | **39** (23 articles + 10 outils + accueil + listing + 3 légales + `a-propos.html`) |
| AdSense · canonical · OG · favicon · manifest · theme-color · meta description · nav · liens légaux | **39/39** sur chaque critère |
| Auteur humain (`Person` en JSON-LD + signature visible) | **23/23 articles** — voir §12.6 |
| JSON-LD `Article` / `BreadcrumbList` / `FAQPage` | 23 / 37 / **32** *(l'accueil n'a pas de fil d'Ariane : normal, c'est la racine)* |
...
3. **✅ Fait le 25/08/2026 — `/simulateur-salaire/` : FAQPage ajouté et erreur de taux corrigée.** ⚠️ **Le diagnostic initial était faux et doit servir de leçon** : la page était décrite comme « la seule sans bloc `tool-seo` », mais elle contenait déjà **1 411 mots** de contenu rédactionnel (un `div.card.seo-section` + une section « Guide pratique »). Elle n'a simplement pas la *classe CSS* `tool-seo`. **Ne pas confondre l'absence d'une classe avec l'absence de contenu** — y ajouter un bloc aurait produit de la redondance pure. Idem pour `/generateur-lettre/`, à vérifier avant d'y toucher.
   - **Vrai manque comblé :** la page affichait 7 `<h3>` formulés en questions mais **aucun balisage `FAQPage`**, alors que les 8 autres outils en avaient un. Bloc JSON-LD ajouté (7 questions, texte fidèle au visible, correspondance vérifiée une à une). Le site passe de 31 à **32 pages** avec `FAQPage`.
   - **🔴 Erreur factuelle trouvée et corrigée sur la page la plus stratégique du site :** elle annonçait encore « assurance maladie (0,75 %) » côté salarié — le taux **supprimé au 01/01/2018**. C'est exactement la correction faite dans `decoder-fiche-paie.html` le 28/07/2026, mais qui avait été **oubliée ici**. Les deux listes de cotisations de la page ont été reprises (maladie 0 %, hors Alsace-Moselle ~1,30 % ; chômage salarial 0 % depuis 2019). Vérifié : plus aucun « 0,75 % » nulle part sur le site. **Leçon : quand un chiffre périmé est corrigé quelque part, le rechercher systématiquement sur les 38 pages** (`grep -rn` sur la valeur), jamais seulement sur la page signalée.
...
6. **✅ Fait le 25/08/2026 — E-E-A-T / YMYL : le site a enfin un auteur humain.** Le contenu fiscal et salarial est classé **YMYL** par Google, qui exige une expertise identifiable. Le site était signé « Aide Salariés », entité sans visage, et les 23 articles déclaraient `"author":{"@type":"Organization"}`. Trois livrables :
   - **`a-propos.html`** (~1 200 mots) — page « Qui sommes-nous » signée **Chapey Rémi**, au sitemap et liée depuis les 39 footers.
